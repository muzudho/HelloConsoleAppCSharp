...
👇　`HelloConsoleAppCSharp/Program.cs` ファイルの `MuzInfrastructureHelper.RunAsync` メソッドの `beforeHostBuild` コールバック関数の中で、`MuzAppSettingsHelper.SetupBeforeHostBuild(～)` を呼び出してください。  
