...
👆 `HelloConsoleAppCSharp/MuzInfrastructureService.cs` ファイルの `MuzInfrastructureService.SetupBeforeBuildAsync()` メソッドの中で、`MuzAppSettingsHelper.SetupBeforeHostBuild(～)` を呼び出してください。
