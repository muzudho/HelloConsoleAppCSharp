...
```csharp
namespace HelloConsoleAppCSharp.Src.Infrastructure.Configuration;
...
using HelloConsoleAppCSharp.Src.Infrastructure.Configuration;
...
👆 `Src/MuzInfrastructureService.cs` ファイルの `MuzInfrastructureService.SetupBeforeBuildAsync()` メソッドの中で、`MuzAppSettingsHelper.SetupBeforeHostBuild(builder)` を呼び出してください。
